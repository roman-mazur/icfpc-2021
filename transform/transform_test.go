...
`
	...
	t.Run("2 squares - fold to bottom", func(t *testing.T) {
		...
		t.Log(f)
		assert.Equal(t, f.Vertices[4].X, 0.0)
		assert.Equal(t, f.Vertices[4].Y, 0.0)
		assert.Equal(t, f.Vertices[5].X, 1.0)
		assert.Equal(t, f.Vertices[5].Y, 0.0)
	})
	...
	t.Run("2 squares - fold to right", func(t *testing.T) {
		...
		t.Log(f)
		assert.Equal(t, f.Vertices[0].X, 2.0)
		assert.Equal(t, f.Vertices[0].Y, 0.0)
		assert.Equal(t, f.Vertices[1].X, 1.0)
		assert.Equal(t, f.Vertices[1].Y, 0.0)
		assert.Equal(t, f.Vertices[2].X, 2.0)
		assert.Equal(t, f.Vertices[2].Y, 1.0)
		assert.Equal(t, f.Vertices[3].X, 1.0)
		assert.Equal(t, f.Vertices[3].Y, 1.0)
		assert.Equal(t, f.Vertices[4].X, 2.0)
		assert.Equal(t, f.Vertices[4].Y, 2.0)
		assert.Equal(t, f.Vertices[5].X, 1.0)
		assert.Equal(t, f.Vertices[5].Y, 2.0)
	})
...
`
	t.Run("2 squares with hand", func(t *testing.T) {
		...
		assert.Equal(t, f.Vertices[6].X, 2.0)
		assert.Equal(t, f.Vertices[6].Y, 2.0)
		assert.Equal(t, f.Vertices[7].X, 2.0)
		assert.Equal(t, f.Vertices[7].Y, 0.0)
	})
...
`
	t.Run("square - diagonal fold", func(t *testing.T) {
		...
		t.Log(f)
		assert.Equal(t, f.Vertices[0].X, 0.0)
		assert.Equal(t, f.Vertices[0].Y, 0.0)
		assert.Equal(t, f.Vertices[1].X, 1.0)
		assert.Equal(t, f.Vertices[1].Y, 0.0)
		assert.Equal(t, f.Vertices[2].X, 1.0)
		assert.Equal(t, f.Vertices[2].Y, 0.0)
		assert.Equal(t, f.Vertices[3].X, 1.0)
		assert.Equal(t, f.Vertices[3].Y, 1.0)
	})
...
`
		...
		eps := 100
		Rotate(&f, f.Edges[0], math.Pi/2, eps)
		assert.Equal(t, f.Vertices[0].X, 1.0)
		assert.Equal(t, f.Vertices[0].Y, 1.0)
		assert.Equal(t, math.Round(f.Vertices[1].X), 1.0)
		assert.Equal(t, f.Vertices[1].Y, 5.0)
		Rotate(&f, f.Edges[0], math.Pi/2, eps)
		assert.Equal(t, f.Vertices[0].X, 1.0)
		assert.Equal(t, f.Vertices[0].Y, 1.0)
		assert.Equal(t, f.Vertices[1].X, -3.0)
		assert.Equal(t, math.Round(f.Vertices[1].Y), 1.0)
		Rotate(&f, f.Edges[0], -math.Pi/2, eps)
		assert.Equal(t, f.Vertices[0].X, 1.0)
		assert.Equal(t, f.Vertices[0].Y, 1.0)
		assert.Equal(t, math.Round(f.Vertices[1].X), 1.0)
		assert.Equal(t, f.Vertices[1].Y, 5.0)
		Rotate(&f, f.Edges[0], math.Pi, eps)
		assert.Equal(t, f.Vertices[0].X, 1.0)
		assert.Equal(t, f.Vertices[0].Y, 1.0)
		assert.Equal(t, math.Round(f.Vertices[1].X), 1.0)
		assert.Equal(t, f.Vertices[1].Y, -3.0)
	})
}
